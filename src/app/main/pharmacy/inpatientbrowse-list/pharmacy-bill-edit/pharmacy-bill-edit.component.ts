...
@Component({
  selector: 'app-pharmacy-bill-edit',
  templateUrl: './pharmacy-bill-edit.component.html',
  styleUrls: ['./pharmacy-bill-edit.component.scss'],
    encapsulation: ViewEncapsulation.None,
    animations: fuseAnimations
})
export class PharmacyBIllEditComponent {
  ...
  constructor(private _matDialog: MatDialog,
    ...
    public _OPListService: OPListService,
     public _InpatientbrowseListService:InpatientbrowseListService,
  ...
  DropDownValue(Obj, contact) {
   ...
   getsaleslist() {
    ...
    var vdata = {
      ...
      "sortOrder": 0,
      "filters": [
       { fieldName: "SalesID", fieldValue: String(this.BillNo), opType: OperatorComparer.Equals },
      { fieldName: "OP_IP_Type", fieldValue: String(this.opD_IPD_Type), opType: OperatorComparer.Equals }
      ],
      "Columns": [],
      "exportType": "JSON"
    }
    this._InpatientbrowseListService.getAllPharmacyBillList(vdata).subscribe(response => {
       this.dsbillList.data = response
      if (this.dsbillList.data.length) {
        this.FinalBillBalAmt = (this.dsbillList.data.reduce((sum, { BalanceAmt }) => sum += +(BalanceAmt || 0), 0)).toFixed(2);
