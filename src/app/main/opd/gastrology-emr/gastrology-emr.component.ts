...
import { CasepaperVisitDetails, gridModel, labRadList, MedicineItemList } from '../new-casepaper/new-casepaper.component';
...
@Component({
  ...
  templateUrl: './gastrology-emr.component.html',
  styleUrls: ['./gastrology-emr.component.scss'],
  encapsulation: ViewEncapsulation.None,
  animations: fuseAnimations
})
export class GastrologyEmrComponent {
  ...
  // QA Part
  displayedQuesColumns: string[] = [
    'QA',
    'Action'
  ]
  displayedQuesselected: string[] = [
    'date',
    'question',
    'buttons'
  ]
  dsQuesList = new MatTableDataSource<QuesList>();
  resultList = new MatTableDataSource<QuesList>();
  ...
  getQuesList() {
    const name = this.myFormGroup.get("quesId").value || "";
    const param = {
      "first": 0,
      "rows": 7,
    ...
    this._CasepaperService.getquList(param).subscribe(Menu => {

      this.dsQuesList.data = Menu.data as QuesList[];
      this.dsQuesList.sort = this.sort;
...
export class QuesList {
  ...
  constructor(QuesList) {
    this.questionName = QuesList.questionName || '';
    this.Price = QuesList.Price || 0;
  }
}