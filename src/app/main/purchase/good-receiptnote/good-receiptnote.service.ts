...
export class GoodReceiptnoteService {
  ...
  getGRNForm() {
    return this._formBuilder.group({
      ...
      MRP: ['', [Validators.required, Validators.min(1),Validators.pattern(/^\d+(\.\d{1,2})?$/)]],
      FreeQty: [0],
      Rate: ['',  [Validators.required, Validators.min(1),Validators.pattern(/^\d+(\.\d{1,2})?$/)]],
      TotalAmount: [0, [Validators.required, Validators.min(1)]],
      Disc: [0, [Validators.min(0),Validators.max(100),Validators.pattern(/^\d+(\.\d{1,2})?$/)]],
      Disc2:  [0, [Validators.min(0),Validators.max(100),Validators.pattern(/^\d+(\.\d{1,2})?$/)]],
      DisAmount: [0],
      DisAmount2: [0],
      GST: [0],
      GSTAmount: [0],
      CGST:[0, [Validators.min(0),Validators.max(100),Validators.pattern(/^\d+(\.\d{1,2})?$/)]],
      CGSTAmount: [0],
      SGST: [0, [Validators.min(0),Validators.max(100),Validators.pattern(/^\d+(\.\d{1,2})?$/)]],
      SGSTAmount: [0],
      IGST:[0, [Validators.min(0),Validators.max(100),Validators.pattern(/^\d+(\.\d{1,2})?$/)]],
      IGSTAmount: [0],
