...
@Component({
  ...
  templateUrl: './in-patient-return.component.html',
  styleUrls: ['./in-patient-return.component.scss'],
  encapsulation: ViewEncapsulation.None,
  animations: fuseAnimations,
})
export class InPatientReturnComponent implements OnInit {
  ...
  constructor(
    public _InPatientReturnService: InPatientReturnService,
  ...
  getSearchList() {
    ...
    if (this._InPatientReturnService.userFormGroup.get('RegID').value.length >= 1) {
      this._InPatientReturnService.getAdmittedpatientlist(m_data).subscribe(resData => {
  ...
  OnRadioChange(){
    ...
    getItemNameList() {
      ...
      console.log(Param)
      this._InPatientReturnService.getCashItemList(Param).subscribe(data => {
      ...
      console.log(Param)
      this._InPatientReturnService.getCreditItemList(Param).subscribe(data => {
        this.ItemfilteredOptions = data;
        console.log(this.ItemfilteredOptions)
        this.filteredOptionsItem = data;
        if (this.ItemfilteredOptions.length == 0) {
        ...
        this.chargeslist.push(
          {
            ...
            ItemName:this._InPatientReturnService.userFormGroup.get('ItemName').value.ItemName || '',
    ...
    onCreditPaySave() {
      ...
      salesReturnHeader['TotalAmount'] = this._InPatientReturnService.IPFinalform.get('FinalTotalAmt').value || 0;
      salesReturnHeader['VatAmount'] = this._InPatientReturnService.IPFinalform.get('FinalGSTAmt').value || 0;
      salesReturnHeader['DiscAmount'] = this._InPatientReturnService.IPFinalform.get('FinalDiscAmount').value || 0;
      salesReturnHeader['NetAmount'] = Math.round(this._InPatientReturnService.IPFinalform.get('FinalNetAmount').value) || 0;
      let PaidAmount
      let balanceAmount
      if(this._InPatientReturnService.userFormGroup.get('TypeofPay').value == 'CashPay'){
        PaidAmount = Math.round(this._InPatientReturnService.IPFinalform.get('FinalNetAmount').value) || 0;
        balanceAmount = 0;
      }else{
        balanceAmount = Math.round(this._InPatientReturnService.IPFinalform.get('FinalNetAmount').value) || 0; 
        PaidAmount = 0;
      } 
      ...
      PaymentInsertobj['CashPayAmount'] = this._InPatientReturnService.IPFinalform.get('FinalNetAmount').value || 0;
   ...
   if(this._InPatientReturnService.userFormGroup.get('TypeofPay').value == 'CashPay'){
    ...
    this._InPatientReturnService.InsertCashSalesReturn(submitData).subscribe(response => {
      if (response) {
    ...
    console.log(submitData);
    this._InPatientReturnService.InsertCreditSalesReturn(submitData).subscribe(response => {
      if (response) {
        console.log(response);
        this.toastr.success('Record Saved Successfully.', 'Save !', {
          toastClass: 'tostr-tost custom-toast-success',
  ...
  OnReset() {
    this._InPatientReturnService.userFormGroup.reset();
    this._InPatientReturnService.IPFinalform.reset();
    this.dsIpSaleItemList.data = [];
    this.chargeslist = [];
    this.vRegNo = '';
    this.vPatientName = '';
    ...
    this.vGenderName =  '';
    this.vAge =  '';
    this._InPatientReturnService.userFormGroup.get('Op_ip_id').setValue('1');
  }
  onClear() {
