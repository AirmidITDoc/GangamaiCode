...
export class CompanyPatientSummaryDashboardComponent implements OnInit {
    ...
    allfilters = [
        ...
        { fieldName: "OPIPType", fieldValue: this.data.row?.opdIpdType ?? 0, opType: OperatorComparer.Equals }
    ]
    ...
    getBilldetails(row) {
        ...
        const m_data = {
            ...
            { "fieldName": "OPIPType", "fieldValue": String(row?.opipType || 0), "opType": "Contains" }
            ],
            "Columns": [],
            "exportType": "JSON"
        }
    ...
    OnPrint(element) {
        if (this.data.row?.opdIpdType == '0') {
            if (element?.salesId > 0) {
                setTimeout(() => {
                    const param = {
                        "searchFields": [
                            { "fieldName": "SalesID", "fieldValue": String(element?.salesId), "opType": "Equals" },
                            { "fieldName": "OP_IP_Type", "fieldValue": String(0), "opType": "Equals" }
                        ],
...
OnPrintStatement(element) {
    ...
    setTimeout(() => {
        ...
        this._AppointmentlistService.getReportView(param).subscribe(res => {
          ...
          this.commonService.Onprint("BillNo", element?.billNo, "IPFinalBillChargesDateWise");
    }
}
