...
export class RadiologyDashboardComponent implements OnInit {
  ...
  pathologistWorkloadColumns: string[] = ['PathologistName', 'TestsReported', 'AvgTime'];
  ...
  // Test Volume by Department Data
  pathologyDepartmentData = [
    { department: 'Hematology', count: 65 },
    { department: 'Biochemistry', count: 82 },
    { department: 'Microbiology', count: 48 },
    { department: 'Serology', count: 39 }
  ];
  ...
  getPathologyDepartmentChart() {
    return new Chart('pathologyDepartmentChart', {
      type: 'bar',
      data: {
        labels: this.pathologyDepartmentData.map(d => d.department),
        datasets: [
          {
            label: 'Number of Tests',
            data: this.pathologyDepartmentData.map(d => d.count),
  ...
  getPathologyVolumeTrendChart() {
    return new Chart('pathologyVolumeTrendChart', {
      type: 'line',
      data: {
        labels: this.pathologyVolumeTrendData.map(d => d.day),
        datasets: [
          {
            label: 'Daily Test Count',
            data: this.pathologyVolumeTrendData.map(d => d.count),
