...
import { EmergencyList } from '../emergency/emergency.component';
...
@Component({
  selector: 'app-emg-settlement',
  templateUrl: './emg-settlement.component.html',
  styleUrls: ['./emg-settlement.component.scss'],
  encapsulation: ViewEncapsulation.None,
  animations: fuseAnimations
})
export class EmgSettlementComponent {
  ...
  gridConfig: gridModel = {
    ...
    sortOrder: 0,
    filters: [
      { fieldName: "RegId", fieldValue: String(this.emgId), opType: OperatorComparer.Equals }
    ]
  }

  constructor(public _EMGSettlementService: EmgSettlementService,
  ...
  getSelectedObj(obj) {
    this.emgId = obj.value;
    setTimeout(() => {
      this._EMGSettlementService.getEmergencyById(this.emgId).subscribe((response) => {
        this.registerObj = response;
        console.log("Emg Detail:", this.registerObj)
        this.PatientName = this.registerObj.firstName + ' ' + this.registerObj.middleName + ' ' + this.registerObj.lastName

      });
    }, 500);
    this.GetDetails(this.emgId)
  }
  ...
  GetDetails(emgId) {
    this.gridConfig = {
      ...
      sortOrder: 0,
      filters: [
        { fieldName: "RegId", fieldValue: String(emgId), opType: OperatorComparer.Equals }
      ]
    }
  ...
  openPaymentpopup(contact) {
    ...
    dialogRef.afterClosed().subscribe(result => {
      if (result && result.IsSubmitFlag) {
        ...
        this._EMGSettlementService.InsertIPSettlementPayment(this.IPBillMyForm.value).subscribe(response => {
          this.GetDetails(this.emgId)
