...
@Component({
    selector: 'airmid-full-timepicker',
    templateUrl: './airmid-full-timepicker.component.html',
    styleUrls: ['./airmid-full-timepicker.component.scss']
    ,
    host: {
        '(focusout)': 'onTouched()',
    },
})
export class AirmidFullTimepickerComponent implements
    ...
    @HostBinding()
    id = `input-control-${++AirmidFullTimepickerComponent.nextId}`;
    ...
    onStartTimeChange(event: string) {
        this.formGroup.controls[this.formStartControlName].setValue(event);
        this.fromTimeChange.emit(event);
    }

    onEndTimeChange(event: string) {
        this.formGroup.controls[this.formEndControlName].setValue(event);
        this.toTimeChange.emit(event);
    }
