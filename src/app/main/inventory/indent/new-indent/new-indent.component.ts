...
export class NewIndentComponent implements OnInit {
  ...
  displayedColumns = ['ItemID', 'ItemName', 'IndentQuantity', 'VerifyQuantity', 'Action'];
  ...
  OnSave() {
    ...
    Swal.fire({
      title: 'Confirm Save',
      text: 'Are you sure you want to save this Indent?',
      ...
      if (this.status == '1') this.IndentSaveFrom.get('priority').setValue(true);
      else this.IndentSaveFrom.get('priority').setValue(false);
      ...
      if (!this.IndentSaveFrom.invalid) {
        ...
        this._IndentService.InsertIndentSave(this.IndentSaveFrom.value).subscribe({
          next: (response) => {
        ...
        if (this.IndentSaveFrom.invalid) {
          for (const controlName in this.IndentSaveFrom.controls) {
            if (this.IndentSaveFrom.controls[controlName].invalid) {
              invalidFields.push(`Indent Form: ${controlName}`);
            }
          }
        }
  ...
  getValidationMessages() {
    return {
      FromStoreId: [{ name: 'required', Message: 'Store Name is required' }],
      ToStoreId: [{ name: 'required', Message: 'Ward Name is required' }],
      ItemName: [],
      Qty: [
  ...
  isValidForm(): boolean {
    const invalidItem = this.dsIndentNameList.data.find((item, index) => {
      if (item.Qty <= 0) {
