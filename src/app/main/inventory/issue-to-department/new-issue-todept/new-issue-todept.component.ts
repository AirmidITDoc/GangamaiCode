...
export class NewIssueTodeptComponent {
  ...
  displayedNewIssuesList1: string[] = ['ItemName', 'BalanceQty', 'Action'];
  displayedNewIssuesList2: string[] = ['BatchNo', 'ExpDateNo', 'BalQty'];
  displayedNewIssuesList3: string[] = ['ItemId', 'ItemName', 'BatchNO', 'ExpDate', 'BalanceQty', 'Qty', 'UnitRate', 'GSTPer', 'GSTAmount', 'TotalAmount', 'Action'];
  ...
  ngOnInit(): void {
    ...
    if (this.IsMaterialAccept) this.Status = 'Material Direct issued with Acceptance';
    else this.Status = 'Material Issued without Acceptance';

    if (this.IsIndentAgainstMaterialAccept) this.Status = ' Indent Against Material issued with Acceptance';
    else this.Status = 'Indent Against  Material  issued without Acceptance';
  ...
  getBatch() {
    ...
    dialogRef.afterClosed().subscribe((result) => {
      ...
      if (result.selectedData) {
        ...
        ((this.vVatAmount = ((this.vTotalAmount * this.vVatPer) / 100).toFixed(4)), (this.vStockId = result.stockId));
  ...
  getFinalCalculation(contact, DraftQty, element) {
    ...
    if (this.RQty && contact.unitMRP) {
      ...
      this.chargeslist.push({
        ...
        TotalAmount:
          (parseFloat((parseInt(this.RQty) * contact.landedRate).toFixed(4)) + parseFloat((((contact.landedRate * contact.vatPercentage) / 100) * parseInt(this.RQty)).toFixed(4))).toFixed(4) || 0, // NetAmt || 0,
        ...
        RoundNetAmt: parseInt(
          (parseFloat((parseInt(this.RQty) * contact.landedRate).toFixed(4)) + parseFloat((((contact.landedRate * contact.vatPercentage) / 100) * parseInt(this.RQty)).toFixed(4))).toFixed(4)
        ),
  ...
  getCellCalculation(contact, Qty) {
    ...
    if (parseFloat(contact.Qty) > parseFloat(contact.BalanceQty)) {
      ...
      if (contact.Qty > 0) {
        ...
        if (contact.IssueBalQty == 0) contact.IsClosed = true;
        else contact.IsClosed = false;
      } else {
        contact.Qty = 0;
        contact.Qty = '';
        contact.VatAmount = 0;
  ...
  OnSave() {
    ...
    Swal.fire({
      title: 'Confirm Save',
      text: 'Are you sure you want to save this Issue to Department?',
      ...
      if (!this.IssueFinalForm.invalid) {
        if (this.vIndentId > 0) {
          if (this.IsIndentAgainstMaterialAccept) this.OnSaveAgaintIndentMaterialAccept();
          else this.OnSaveAgaintIndent();
        } else {
          if (this.IsMaterialAccept) this.OnNewAcceptSave();
          else this.OnNewSave();
        }
      } else {
        const invalidFields = [];
  ...
  OnNewAcceptSave() {
    ...
    this._IssueToDep.IssuetodepAcceptMaterialSave(this.NewIssueGroupAccept.value).subscribe({
      next: (response) => {
  ...
  OnNewSave() {
    ...
    console.log(this.FinalIssueForm.value);
    this._IssueToDep.IssuetodepSave(this.FinalIssueForm.value).subscribe({
      next: (response) => {
        this.viewgetIssuetodeptReportPdf(response);
        this.isSaving = false;
        this._matDialog.closeAll();
  ...
  OnSaveAgaintIndent() {
    ...
    this.dsNewIssueItemList.data.forEach((element) => {
      console.log(element);
      if (this.CellCalculation == 0) console.log(element);
      debugger;
      const balQty = parseInt(element.IndQty) - parseInt(element.Qty);

      if (balQty == 0) element.IsClosed = true;
      else element.IsClosed = false;
    ...
    this.dsNewIssueItemList.data.forEach((item) => {
      if (item.IsClosed) this.Isclosedchk = true;
      else this.Isclosedchk = false;
    ...
    this._IssueToDep.IssuetodepAgaintIndetSave(this.FinalIssueaginstForm.value).subscribe({
      next: (response) => {
        this.viewgetIssuetodeptReportPdf(response);
        this.isSaving = false;
        this._matDialog.closeAll();
  ...
  OnSaveAgaintIndentMaterialAccept() {
    ...
    this._IssueToDep.IssuetodepAgaintIndetdirectAcceptSave(this.FinalIssueaginstAcceptForm.value).subscribe({
      next: (response) => {
        this.viewgetIssuetodeptReportPdf(response);
        this.isSaving = false;
        this._matDialog.closeAll();
  ...
  isValidForm(): boolean {
    const invalidItem = this.dsNewIssueItemList.data.find((item, index) => {
      if (item.Qty <= 0) {
        this.toastr.warning(`Row ${index + 1}: Quantity must be greater than 0`, 'Warning !', { toastClass: 'tostr-tost custom-toast-warning' });
        return true;
      }
      return false;
    });
