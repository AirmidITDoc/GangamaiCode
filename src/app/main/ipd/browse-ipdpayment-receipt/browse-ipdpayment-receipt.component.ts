...
export class BrowseIPDPaymentReceiptComponent implements OnInit {
  ...
  onExport(exprtType){
    ...
    //   }
    //   var fileName = "Indoor-Payment Receipt-List " + new Date() +".xlsx";
    //   if(exprtType =="Excel"){
  ...
  //     let keysArray = ['HospitalName','HospitalAddress','RegNo', 'ReceiptNo','PaymentDate','PatientName','PBillNo','PaidAmount','Remark','BillDate','PaymentId','TotalAmount','CashPayAmount','UserName']; // resData[0].TempKeys;
  ...
  //         console.log(this.printTemplate.replace(re, this.reportPrintObj[keysArray[i]]))
  //       }
...
getTemplate() {
  ...
  this._BrowseIPDPaymentReceiptService.getTemplate(query).subscribe((resData: any) => {
    this.printTemplate = resData[0].TempDesign;
    let keysArray = ['HospitalName','HospitalAddress','Phone','EmailId','RegId', 'ReceiptNo','PaymentDate','PatientName','AgeDay','AgeMonth','AgeYear','PBillNo','PaidAmount','Remark','BillDate','PaymentId','TotalAmt','CashPayAmount','UserName'
  ,'CashPayAmount','CardPayAmount','ChequePayAmount','NEFTPayAmount','PayTMAmount','Remark','UserName','CardNo','CardBankName'];
      ...
      this.printTemplate = this.printTemplate.replace('StrPrintDate', this.transform2(this.reportPrintObj.BillDate));
