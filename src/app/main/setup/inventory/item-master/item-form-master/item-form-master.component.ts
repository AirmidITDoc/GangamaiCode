...
export class ItemFormMasterComponent implements OnInit {
    ...
    ngOnInit(): void {
      this.itemForm = this._ItemMasterService.createItemmasterForm();
      var m_data = {
        currencyId: this.data?.currencyId,
        currencyName: this.data?.currencyName.trim(),
        isDeleted: JSON.stringify(this.data?.isActive),
      };
      this.itemForm.patchValue(m_data);
    }
    ...
    if (this.itemForm.invalid) {
       ...
       return;
        ...
        if(this.itemForm.valid) {
          this._ItemMasterService.itemMasterSave(this.itemForm.value).subscribe((response) => {
